...
class URDFStep():
    def __init__(self, name, type, Tshift, nlocal):
        ...
        self.link_lengths = None # Length of each link between non-FIXED joints
...
class KinematicChain():
    ...
    def fkin(self, q, get_link_lengths=False):
        ...
        for step in self.steps:
            ...
            if step.type is JointType.REVOLUTE:
                ...
                R_rotary = Rotn(step.nlocal, q[step.dof])
                p_rotary = pzero()
                T_rotary = T_from_Rp(R_rotary, p_rotary)
                
                T = T @ step.Tshift @ T_rotary
            elif step.type is JointType.LINEAR:
                # Linear is a shift followed by a translation:
                R_prismatic = Reye()
                p_prismatic = step.nlocal * q[step.dof]
                T_prismatic = T_from_Rp(R_prismatic, p_prismatic)
                
                T = T @ step.Tshift @ T_prismatic
            else:
        ...
        # TODO: It's annoying to recalculate the link lengths every time `fkin`
        # is called, but I don't know a quick fix to this issue
        link_lengths = []
        for i in range(len(p) - 1):
            link_length = np.linalg.norm(p[i + 1] - p[i])
